...
if (isset($_GET['supp_user'])) {
    ...
    $stmt = $pdo->prepare("SELECT id, enseigne_id FROM utilisateurs WHERE id = ? AND enseigne_id = ?");
    $stmt->execute([$id, $enseigne_id]);
    $userToDelete = $stmt->fetch();

    if ($userToDelete && $_SESSION['user_id'] != $id) {
        $pdo->prepare("DELETE FROM utilisateurs WHERE id = ? AND enseigne_id = ?")->execute([$id, $enseigne_id]);
        $success = "✅ Utilisateur supprimé avec succès.";
    }
}

// ── STATS ────────────────────────────────────────────────────
$stmt = $pdo->prepare("SELECT COUNT(*) FROM utilisateurs WHERE enseigne_id = ?");
$stmt->execute([$enseigne_id]);
$nb_users = $stmt->fetchColumn();

// TEMPORAIRE : ces tables ne semblent pas encore avoir enseigne_id
...
// Si produits.enseigne_id existe, tu peux garder cette requête.
// Sinon remplace par SELECT COUNT(*) FROM mouvements
$stmt = $pdo->prepare("
    SELECT COUNT(*)
    FROM mouvements m
...
// ── UTILISATEURS DE L'ENSEIGNE ───────────────────────────────
$stmt = $pdo->prepare("
    SELECT u.*, e.nom AS enseigne_nom
    FROM utilisateurs u
    LEFT JOIN enseignes e ON u.enseigne_id = e.id
    WHERE u.enseigne_id = ?
...
// ── ENSEIGNE ACTIVE ──────────────────────────────────────────
$stmt = $pdo->prepare("SELECT * FROM enseignes WHERE id = ? ORDER BY nom");
$stmt->execute([$enseigne_id]);
$enseignes = $stmt->fetchAll();
?>
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
    ...
    .role-employe, .role-chef_rayon, .role-magasinier { background:#dcfce7; color:#166534; }
...
<div class="container">
  ...
  <div class="kpi-grid">
    <div class="kpi"><div class="kpi-val"><?= $nb_enseignes ?></div><div class="kpi-lbl">Enseigne</div></div>
  ...
  <div class="card">
    <div class="card-header">Enseigne active (<?= count($enseignes) ?>)</div>
    <table>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            <span class="role-badge role-<?= h($user['role']) ?>"><?= strtoupper(h($user['role'])) ?></span>
          </td>
          ...
          <td>
            <?php if ($_SESSION['user_id'] != $user['id']): ?>
              <a href="?supp_user=<?= $user['id'] ?>" class="btn-delete" onclick="return confirm('Supprimer <?= h($user['prenom']) ?> ?')">
                Supprimer
              </a>
            <?php else: ?>

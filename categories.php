...
if (isset($_POST['ajouter'])) {
    ...
    if ($nom === '') {
        ...
        try {
            $pdo->prepare("INSERT INTO categories (nom, enseigne_id) VALUES (?, ?)")
                ->execute([$nom, $enseigne_id]);
...
if (isset($_POST['modifier'])) {
    ...
    if ($nom === '') {
        ...
        try {
            $pdo->prepare("UPDATE categories SET nom = ? WHERE id = ? AND enseigne_id = ?")
                ->execute([$nom, $id, $enseigne_id]);
...
if (isset($_GET['supprimer']) && canDelete()) {
    ...
    $nb = $pdo->prepare("SELECT COUNT(*) FROM produits WHERE categorie_id = ? AND enseigne_id = ?");
    $nb->execute([$id, $enseigne_id]);

    if ($nb->fetchColumn() > 0) {
        $error = "Impossible : cette catégorie contient des produits.";
    } else {
        $pdo->prepare("DELETE FROM categories WHERE id = ? AND enseigne_id = ?")
            ->execute([$id, $enseigne_id]);
          ...
          <tr>
            ...
            <td>
              ...
              <?php if (canDelete() && $cat['nb_produits'] == 0): ?>
                <a href="?supprimer=<?= $cat['id'] ?>" class="btn-sm btn-delete" onclick="return confirm('Supprimer <?= h($cat['nom']) ?> ?')">Supprimer</a>
              <?php endif; ?>
            </td>
          </tr>
          <?php endforeach; ?>

...
// ── Statistiques filtrées par enseigne ──────────────────────
$s = $pdo->prepare("SELECT COUNT(*) FROM produits WHERE enseigne_id = ?");
$s->execute([$enseigne_id]);
$totalProduits = (int)$s->fetchColumn();

$totalCategories = (int)$pdo->query("SELECT COUNT(*) FROM categories")->fetchColumn();

$s = $pdo->prepare("SELECT COUNT(*) FROM produits WHERE enseigne_id = ? AND quantite > 0 AND quantite <= seuil_alerte");
$s->execute([$enseigne_id]);
$stockFaible = (int)$s->fetchColumn();

$s = $pdo->prepare("SELECT COUNT(*) FROM produits WHERE enseigne_id = ? AND quantite = 0");
$s->execute([$enseigne_id]);
$ruptures = (int)$s->fetchColumn();

$s = $pdo->prepare("SELECT SUM(prix * quantite) FROM produits WHERE enseigne_id = ?");
$s->execute([$enseigne_id]);
$valeurStock = (float)($s->fetchColumn() ?: 0);

// ── Derniers mouvements filtrés par enseigne ────────────────
$s = $pdo->prepare("
...
");
$s->execute([$enseigne_id]);
$derniersMouvements = $s->fetchAll();

// ── Produits en alerte filtrés par enseigne ─────────────────
$s = $pdo->prepare("
    ...
    WHERE p.enseigne_id = ? AND p.quantite <= p.seuil_alerte
    ORDER BY p.quantite ASC LIMIT 6
");
$s->execute([$enseigne_id]);
$produitsAlerte = $s->fetchAll();

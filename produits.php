...
if (isset($_POST['ajouter']) || isset($_POST['modifier'])) {
    ...
    if (!$catCheck->fetch()) {
        $error = "Catégorie invalide pour cette enseigne.";
    } else {
        if (isset($_POST['ajouter'])) {
            try {
                $pdo->prepare("
                    INSERT INTO produits
                    (reference, nom, marque, fournisseur, categorie_id, quantite, seuil_alerte, prix, image, enseigne_id)
                    VALUES (?,?,?,?,?,?,?,?,?,?)
                ")->execute([$ref, $nom, $marque, $fournisseur, $cat, $qte, $seuil, $prix, $imagePath, $enseigne_id]);

                $success = "Produit <strong>" . h($nom) . "</strong> ajouté !";
            } catch (PDOException) {
                $error = "Référence déjà utilisée ou données invalides.";
            }
        } else {
            $pdo->prepare("
                UPDATE produits
                SET reference=?, nom=?, marque=?, fournisseur=?, categorie_id=?, quantite=?, seuil_alerte=?, prix=?, image=?
                WHERE id=? AND enseigne_id=?
            ")->execute([$ref, $nom, $marque, $fournisseur, $cat, $qte, $seuil, $prix, $imagePath, $id, $enseigne_id]);
...
if (isset($_GET['supprimer']) && canDelete()) {
    $pdo->prepare("DELETE FROM produits WHERE id=? AND enseigne_id=?")->execute([(int)$_GET['supprimer'], $enseigne_id]);
...
<head>
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  .search-row{display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem;gap:1rem;flex-wrap:wrap;}
  .search-input{padding:10px 14px;border-radius:10px;border:1px solid var(--border);font-family:inherit;font-size:14px;outline:none;width:300px;max-width:100%;}
      ...
      <tbody id="productTable">
        ...
        <tr class="product-row">
          ...
          <td style="text-align:right;">
            ...
            <?php endif; ?>
            <?php if (canDelete()): ?>
              <a href="?supprimer=<?= $p['id'] ?>" class="btn-sm btn-delete" onclick="return confirm('Supprimer <?= h($p['nom']) ?> ?')">
                🗑️ Supprimer
              </a>
            <?php endif; ?>
          </td>

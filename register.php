...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $step === '2') {
    ...
    if ($mode === 'create') {
        ...
        if (empty($erreurs)) {
            ...
            // 2. Copier les produits de l'enseigne de référence (id=1)
            $produits_ref = $pdo->query("SELECT * FROM produits WHERE enseigne_id = 1")->fetchAll();
            $insert_produit = $pdo->prepare("
                INSERT INTO produits 
                (enseigne_id, reference, nom, marque, fournisseur, categorie_id, quantite, seuil_alerte, prix, image)
                VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, '')
            ");
            foreach ($produits_ref as $p) {
            ...
            $_SESSION['code_invitation'] = $codeInvit;
